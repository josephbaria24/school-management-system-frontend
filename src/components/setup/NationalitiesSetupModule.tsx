...
export function NationalitiesSetupModule() {
  ...
  const handleSave = async () => {
    ...
    if (!nationality_name) {
      toast({
        ...
        description: "Nationality name is required.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${API}/api/nationalities/${selectedId}`, {
        method: "DELETE",
      });
      const result = await res.json().catch(() => ({}));
          ...
          <div className="w-[430px] border border-[#79b898] bg-white mb-2">
            ...
            <div className="p-2 space-y-1.5">
              <div className="flex items-center gap-2">
                <Label className="w-28 text-right text-[11px]">Nationality ID</Label>
                <Input className="h-6 text-[11px] w-[160px]" value={form.id_text} readOnly />
              </div>
              <div className="flex items-center gap-2">
                <Label className="w-28 text-right text-[11px]">Nationality Name</Label>
                <Input className="h-6 text-[11px] w-[250px]" value={form.nationality_name} onChange={(e) => setForm((s) => ({ ...s, nationality_name: e.target.value }))} />
              </div>
              <div className="flex items-center gap-2">
            ...
            <div className="max-h-[420px] overflow-auto">
              {rows.map((r, idx) => (
                ...
                >
                  ...
                  <div className="col-span-6 px-2 py-0.5 border-r border-[#d8ebdf] truncate">{r.nationality_name}</div>
